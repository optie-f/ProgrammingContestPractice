...
- diverta2019_2: 約数のみに限る、とまで絞れたら、あとはそれぞれ判定すればよい
...
## AGC
- agc035_a: A - XOR Circle: a ⊕ a = 0 なので、「両隣の数の排他的論理和が自身に等しい」というのは「連続する3つの数の排他的論理和がゼロ」に等しく、そうなるのはすべてゼロか、総数が 3 の倍数であり、かつN/3個の0とそれ以外のxからのみなるか、かつx ⊕ y ⊕ z = 0 が成立するような 3 つの相異なる整数 x, y, z がN/3個ずつかの三通りしかない。å
- agc033_a: xに対してW, yに対してH. 変数を間違えない
- agc032 a: A - Limited Insertion. 「挿入操作の繰り返し」は逆から見れば「除去操作の繰り返し」.
- agc031_a: A - Colorful Subsequence. ここでいう部分列は連続している必要がないので, `2^N-1` 通り全てが区別される. そこで, それぞれの色について, その出現位置のどこかから選ぶか, どこからも選ばないかということを考えると, Π(各色の出現回数+1) - 1 となる.
- agc023_a: Zero-sum ranges. 数列の部分列の総和と見たら累積和の差分を考える. 部分列の総和がゼロとは, 累積和列の二要素の差分がゼロということであり、つまり0を含む累積和列のなかで同じ数の組を数えればよい.
- agc018_A: Getting Difference. **差は約数に二数の最小公倍数を持つ**
  - a,b の最小公倍数を g とすると a=mg, b=ng で a-b = (m-n)\*g となるので, a と b の差は a と b の最小公倍数の倍数でなければならない.
  - また, a,b,c の最小公倍数は a, b の最小公倍数の約数になる.
  - そこから, 入力全部の最小公倍数をとって、その倍数であるかどうかを見ればよいということになる.（あと最大値を超えていないこと）

## ARC

- arc041_b: アメーバ: ラスタスキャンしながらアメーバの所在を確定させていく. 確実にわかるところはどこか、わかるところから決めていけないかと考える.
- arc037_b: バウムテスト=木の判定. ある頂点の親がなんだったかを保持しておき, 到達済み頂点 u と接続された頂点 v について, u の親が v であるという情報がないのに v が訪問済みなら木ではない
- arc031_2: B:埋め立て. ループのH,Wを定数にする場合、本当にそれで正しいか再三確認する. 何度も幅優先探索を回す場合, 初期化は正しいか.
- arc004_2: 2 点間距離の最大と最小. 最小値は, 2 回まで適当な点で折ることを考えると、`d_max - (d_sum - d_max)` と 0 のうち大きい方が答えとなる

## ABC
- abc166_e:  This Message Will Self-Destruct in 5s: `j-i=A[i]+A[j]`を`j-A[j]=i+A[i]` とすれば、両辺それぞれを連想配列で持ってペアを簡単に計算できる. **(i,jの式)=(i,jの式) を (iの式)=(jの式) に変形すると楽になることがある**.
- abc165_d: Floor Function: 数学っぽい問題では小さい数で全列挙して実験する
- abc164_d: `a=b (mod p)` のとき `a-b=0(mod p)` であり `10^n x (a-b)=0 (mod p)` である. そのため, 累積和のように, 「k+1 文字目以降を整数とみなしたときの剰余 = `T_k`」を考えて差分をとることを想定して, `T_k` の個数ごとに組を考えるという戦略が可能である. この `T_k` はDPで求まる.
- abc162_e: gcd が K の倍数である必要十分条件は 引数がすべて K の倍数であること. とりうる最大値からDP的に始めて, 2K, 3K である場合を引いていくことで, Kジャストの個数を求められる.
- abc162_f: DPテーブルの初期値に気を付ける. ループで触れないすべてのパターンを確かめておく.
- abc161_d: ルンルン数. 再帰をかけると深さ優先探索になってしまい今回の要求とはあわない. 幅優先探索と Queue は等価
- abc161_f:Division or Substraction. 割り算をするパタンとしないパタンで分ける. 割り切れる->約数. N mod K = p となる K は N - p の約数.
- abc160_d: X-Y を通らないパタン、i→X→Y→j、i→Y→X→j にわけて考えると、`|i-j|` か `|X-i| + 1 + |j-Y|` か `|Y-i| + 1 + |X-j|` のどれかしかないことがわかる
- abc160_e: Red and Green Apples. 赤と緑それぞれ上位 X,Y 個だけ残して、そこに無色のりんごを加えたものから上位 X+Y をとればよい(適切に着色すればどうとでもなるので)
...
- abc148_e: 2,4,6..N の積の末尾ゼロの個数は素因数の 10 の個数、つまり素因数中の 2, 5 の数の小さい方. ここで、2,4,...N は明らかに 2 のほうがおおいので 5 の数を数える. 5 を因数に複数もつ数を考慮すると、 5 で割り切れる数 + 5x5 で割り切れる数 + 5x5x5 で割り切れる数 の総和がその答えになる. そして末尾が 5 になる数が存在しない性質から、 N/10, N/50, N/250 を見ていけばよい.
- abc147_d: xor と加算について分配則が成り立ったりはしない. xor は各ビット独立に計算される. - abc145_c:`next_permutation( begin(v), end(v) )`は、辞書順の上でひとつずつ最後まで並び替える
- abc143_c: 二分探索の初期値は両端ともに開区間端にしておくべき
...
- abc085_d: Katana Thrower. 投げる専用刀と振る専用刀がそれぞれあると思っても構わない.
