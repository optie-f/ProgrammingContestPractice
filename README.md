...
abc034_c: 剰余の世界での割り算の方法. **フェルマーの小定理** により, pと互いに素な a に対して `a^(p-1)≡1 (mod p)` であることから `a^(p-2)≡a^(-1) (mod p)` となる. つまりmod p の世界では a の乗法逆元は `a^(p-2)` になる. また, `a^b` は, bが偶数なら `d=a^(b/2); return d*d;` とし, 奇数なら `a * a^(b-1)`という再帰関数をかけば log(b) で計算できる. \
